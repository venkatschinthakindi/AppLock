...
fun SubscriptionScreen() {
    ...
    Column(Modifier.fillMaxSize().padding(24.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        ...
        Button(enabled = activity != null && !pro, onClick = { activity?.let(billing::launchPurchase) }, Modifier.fillMaxWidth()) { Text(if (pro) "Subscription active" else "Continue with Google Play") }
    }
}
